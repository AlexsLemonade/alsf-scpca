...
length(gene_overlap)/(length(cr_like_em_genes))
```

When you look at shared cells only, `cr-like` and `cr-like-em` identify the exact same genes. 
However, we had seen an increase in the genes detected/cell. 
It looks like the actual number of genes doesn't increase, but the coverage of some genes across each the cells increases, leading to a slight increase in the number of genes/cell with `cr-like-em`. 

```{r}
ggplot(all_rowdata_filtered, aes(x = alevin_resolution, y = detected)) +
  geom_boxplot()+
  facet_grid(~ sample) +
  theme_classic() +
  coord_cartesian(ylim = c(0,10)) +
  ylab("Percent of Cells Gene Detected In")
```
Here we can see that across the board, the `cr-like-em` resolution results in what appears to be an increase in the percentage of cells genes are detected in when compared to `cr-like`. 
Does this still hold if we remove genes that are only detected in 5% of cells? 

```{r}
ggplot(all_rowdata_filtered %>%
         dplyr::filter(detected > 5), aes(x = alevin_resolution, y = detected)) +
  geom_boxplot()+
  facet_grid(~ sample) +
  theme_classic() +
  #coord_cartesian(ylim = c(0,10)) +
  ylab("Percent of Cells Gene Detected In")
```
If we only look at genes detected in > 5% of cells we no longer see a clear difference between the two resolutions. 
