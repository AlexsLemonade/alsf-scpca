...
quant_info <- quant_info_table(data_dir = file_dir,
                 ...
                 samples = c('SCPCR000126','SCPCR000127','SCPCR000220','SCPCR000221')) %>%
...
sample_names <- c('SCPCR000126','SCPCR000127', 'SCPCR000220', 'SCPCR000221')
...
readr::write_rds(filtered_sces, file.path(results_dir, "alevin-fry-cr-like-em-emptydrops-100-sces.rds"))

emptydrops_filter_sces_200 <- purrr::map(fry_sces, ~ filter_counts(sce = .x, lower = 200))
readr::write_rds(filtered_sces, file.path(results_dir, "alevin-fry-cr-like-em-emptydrops-200-sces.rds"))
```
...
cr_filter_cell_num <- crlike_filtered_sces %>%
  purrr::map(get_num_cells) %>%
  as.data.frame()
...
names(cellranger_sces) <- sample_names
...
ggplot(combined_cell_num_df, aes(x = sample, y = number_cells, color = filtering_method)) + 
  geom_point(size = 3)
```
Here you can see that `emptyDropsCellRanger()` doesn't appear much better than just `emptyDrops()` and they both are still reporting higher cell numbers than Cell Ranger. 
Additionally, as expected, using the knee-based method decreases the numbers of cells across the board. 
It appears that `emptyDrops()` with `lower=200` is the most comparable to Cell Ranger for all samples and we see that those cell numbers tend to be fairly similar. 
...
crlike_filtered_sces <- crlike_filtered_sces %>%
  purrr::map(~ add_cell_mito_qc(.x, mito = mito_genes))
...
```
Here you can see that the median UMI/cell and genes detected/cell look lower for Alevin-fry with `emptyDrops` and `emptyDropsCellRanger` than Cell Ranger, which is consistent with the previous finding that they are reporting more cells than Cell Ranger. 
However, if we use that threshold of `lower=200` for `emptyDrops`, we see a much more similar distribution and median of UMI/cell and genes detected/cell. 
...
```
It appears that the majority if the cell barcodes in Cell Ranger are also found in our filtered sces (which is a good sign), and over 75% of the cell barcodes that we see in the filtered sce are also found in Cell Ranger. 
