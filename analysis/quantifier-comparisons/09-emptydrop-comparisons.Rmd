...
```{r}
s3_dir <- 's3://nextflow-ccdl-results/scpca/alevin-fry-unfiltered-quant/v0.3.0'
local_dir <- file.path(file_dir, "alevin-fry-unfiltered")

sync_call <- paste('aws s3 cp', s3_dir, local_dir, 
                         '--exclude "*"', '--include "SCPCR000119-spliced_intron_txome_k31-salign-cr-like/alevin/quants_mat.mtx"', '--exclude "*.rad"',
                         '--recursive')
system(sync_call, ignore.stdout = TRUE)
```
