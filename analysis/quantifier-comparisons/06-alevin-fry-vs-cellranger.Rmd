...
```{r}
# use addCellQC function to addPerCellQC with gene detection threshold
# only genes with detection in > 5% of cells will be included in the QC calculations
sce_list <- sce_list %>%
  purrr::map(.f =  
               ~ purrr::walk(.x, addCellQC, mito = mito_genes, threshold = 5
  ))
```
...
names(sce_list) <- c(rep("alevin-fry", 8), "cellranger")

# merge back into a dataframe for plotting 
coldata_threshold <- purrr::map_df(
  sce_list,
  ~ purrr::map_df(.x, coldata_to_df, .id = "quant_id"), 
  .id = "tool"
)
