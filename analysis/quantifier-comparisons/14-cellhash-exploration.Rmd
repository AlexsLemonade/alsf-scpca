...
These are unfiltered, so we will filter to likely cells with `emptyDropsCellRanger`, which should perform filtering similar to Cell Ranger.
...
Let's reduce our hash data to just those & replaces the sample ids:
```{r}
...
There does appear to be some correspondance between clusters and classification, which could make sense, so maybe it is not all lost!
