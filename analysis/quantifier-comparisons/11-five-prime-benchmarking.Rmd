...
```{r}
fry_filtering <- c("knee", "unfiltered")
...
coldata_info_df <- coldata_df %>%
  ...
  dplyr::left_join(quant_info,
                   by = c("tool" = "tool", 
                          "quant_id" = "quant_dir")) %>%
  dplyr::mutate(filter_strategy = ifelse(filter_strategy %in% fry_filtering, filter_strategy, "cellranger"))
```
...
In order to see if there are any differences between the two Alevin-fry filtering strategies in the ability to assign reads to each cell barcode, we first are looking at all cells before restricting to shared cells only. 

```{r}
# mito content in shared cells
ggplot(coldata_info_df, aes(x = tool, y = subsets_mito_percent, fill = filter_strategy)) + 
  geom_boxplot() + 
  facet_grid(~ sample) +
  theme_classic() + 
...
```{r}
ggplot(coldata_info_df, aes(x = tool, y = sum, fill = filter_strategy)) + 
  geom_boxplot() + 
  facet_grid(~ sample) +
  theme_classic() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
...
```{r}
ggplot(coldata_info_df, aes(x = tool, y = detected, fill = filter_strategy)) + 
  geom_boxplot() + 
  facet_grid(~ sample) +
  theme_classic() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
  ...
  ylim(c(0,6000))
```
Here, we see that the knee filtering doesn't provide an advantage over use of the unfiltered permit list, and regardless of filtering the counts and genes detected/cell are lower than with Cell Ranger. 
It appears that these differences are due to an issue with alignment and mapping in Alevin-fry and not with cell barcode assignment.
...
```{r}
# mito content in shared cells
ggplot(coldata_qc_common, aes(x = tool, y = subsets_mito_percent, fill = filter_strategy)) + 
  geom_boxplot() + 
  facet_grid(~ sample) +
  theme_classic() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
...
```
```{r}
ggplot(coldata_qc_common, aes(x = tool, y = sum, fill = filter_strategy)) + 
  geom_boxplot() + 
  facet_grid(~ sample) +
  theme_classic() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
...
```
```{r}
ggplot(coldata_qc_common, aes(x = tool, y = detected, fill = filter_strategy)) + 
  geom_boxplot() + 
  facet_grid(~ sample) +
  theme_classic() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
  ...
  ylim(c(0,6000))
```
Interestingly we see that alevin-fry shows higher mito content per cell and lower overall counts and number of genes detected per cell than Cell Ranger even in shared cells. 
...
```{r}
ggplot(qc_common_threshold, aes(x = tool, y = sum, fill = filter_strategy)) + 
  geom_boxplot() + 
  facet_grid(~ sample) +
  theme_classic() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
...
```
```{r}
ggplot(qc_common_threshold, aes(x = tool, y = detected, fill = filter_strategy)) + 
  geom_boxplot() + 
  facet_grid(~ sample) +
  theme_classic() + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
...
rowdata_cor %>% 
  ...
  dplyr::summarize(
    alevin_fry_knee_cellranger_cor = cor(`cellranger_cellranger`, `alevin-fry_knee`, method = "spearman"),
    alevin_fry_unfiltered_cellranger_cor = cor(`cellranger_cellranger`, `alevin-fry_unfiltered`, method = "spearman")
  )
```
```{r}
ggplot(rowdata_cor, aes(x = `cellranger_cellranger`, y = `alevin-fry_unfiltered`)) +
  geom_point(size = 0.5, alpha = 0.1) + 
  facet_wrap(~ sample) + 
  scale_x_log10() + 
  scale_y_log10() + 
  labs(x = "Cell Ranger mean gene expression", y = "Alevin Fry, unfiltered permit list, Mean gene expression") + 
  theme_classic()
```

```{r}
ggplot(rowdata_cor, aes(x = `cellranger_cellranger`, y = `alevin-fry_knee`)) +
  geom_point(size = 0.5, alpha = 0.1) + 
  facet_wrap(~ sample) + 
  scale_x_log10() + 
  scale_y_log10() + 
  labs(x = "Cell Ranger mean gene expression", y = "Alevin Fry, Knee filtering, Mean gene expression") + 
  theme_classic()
```


Unlike with the 3' libraries, the 5' libraries have fairly poor correlation between Alevin-fry and Cell Ranger, regardless of using knee or the permit list option. 
