...
residuals_df <- mean_exp_df_spread %>%
  ...
  dplyr::mutate(
    cellranger_cr_like = lm(cellranger ~ `cr-like`)$residuals,
    cellranger_cr_like_em = lm(cellranger ~ `cr-like-em`)$residuals
  ) %>%
  dplyr::left_join(mean_exp_df_spread) %>%
  # condense all residuals for each tool comparison into one column for easy plotting
  tidyr::pivot_longer(cols = c("cellranger_cr_like", "cellranger_cr_like_em"),
                      names_to = "tool_comparisons", 
                      values_to = "residuals") %>%
  tidyr::pivot_longer(cols = c("cellranger", "cr-like", "cr-like-em"),
                      names_to = "tool", 
                      values_to = "mean_log_expression")
```

```{r fig.height=10, fig.width=10}
ggplot(residuals_df, aes(x = mean_log_expression, y = residuals)) + 
  geom_point(size = 0.5) + 
  facet_wrap(tool ~ sample, nrow = 3)+ 
  # label genes that have abs(residual) > 1 to see which genes are consistently labeled across samples
  geom_text_repel(data = residuals_df %>%
              dplyr::filter(abs(residuals) > 1), 
            mapping = aes(x = mean_log_expression, y = residuals, label = gene_id),
...
residuals_df %>%
  ...
  dplyr::select(Symbol, gene_id, residuals, `cr-like`, `cr-like-em`, cellranger)
```
If you scroll through the list of genes that have abs(residuals) > 1 the gene list is mostly made up of ribosomal genes.
