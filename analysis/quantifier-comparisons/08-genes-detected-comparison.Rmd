...
all_sces <- list(cr_like_sce,
                 cr_like_em_sce,
                 cellranger_sce)
names(all_sces) <- c('cr-like', 'cr-like-em', 'not_alevin')
...
cr_like_genes <- rowdata_filtered %>%
  dplyr::filter(alevin_resolution == "cr-like" & detected > 0 & mean > 0) %>%
  dplyr::pull(gene_id) %>%
  unique()

cr_em_genes <- rowdata_filtered %>%
  dplyr::filter(alevin_resolution == "cr-like-em" & detected > 0 & mean > 0) %>%
  dplyr::pull(gene_id) %>%
  unique()
```
...
cr_like_overlap <- rowdata_filtered %>%
  dplyr::filter(gene_id %in% cr_em_genes & alevin_resolution %in% c("cr-like", "not_alevin")) %>%
  dplyr::pull(gene_id) %>%
  unique()
cr_like_unique <- setdiff(cr_em_genes, cr_like_overlap)
```


```{r}
# visualize overlap between gene sets based on tool
rowdata_filtered[which(rowdata_filtered$alevin_resolution == "not_alevin"), "alevin_resolution"] <- "cellranger"
...
gene_detect_df %>%
  ...
  ggplot(aes(x = tools_detected)) +
  geom_bar() +
  scale_x_upset(n_intersections = 6)
```


When looking at shared cells only and genes that are expressed and detected > 0, we see that alevin-fry detects more genes than cellranger in both methods. 
Almost every gene that is found in cellranger is also being found in Alevin-fry so we are probably not losing much.
We also see here that there are more in `cr-like-em` than in `cr-like`. 
...
ggplot(mean_exp_df, aes(x = alevin_resolution, y = mean, fill = gene_status)) + 
  geom_boxplot() + 
  facet_grid(~sample) + 
...
Let's look at the few unique genes in `cr-like-em` and cellranger that are higher expressed. 
 
```{r}
cellranger_unique_high <- mean_exp_df %>%
  dplyr::filter(alevin_resolution == "not_alevin" & 
                  gene_status == "unique" & 
                  mean > 0.25) %>%
  dplyr::pull(gene_id)
...
ENSG00000244398 - RP11

These all appear to be expressed in a majority of the cells. 
The RPL genes represent ribosomal genes that are likely not biologically significant. 
...
cr_em_lost <- rowdata_filtered %>%
  dplyr::filter(gene_id %in% cr_like_unique) %>%
...
It looks like we are getting all the same variation of a similar pathway with 32 genes that are found to be involved in sensory perception, which makes sense given the amount of olfactory genes that are detected. 

As a reminder, these are the genes that are not being detected by alevin-fry using either method so using alevin-fry would mean losing this list of genes, but the low expression doesn't seem cause for worry. 

What about the type of genes that are unique to `cr-like-em`. 

```{r}
cr_em_lost_gene_list <- cr_em_lost$gene_id %>%
  unique()
...
```{r}
go_ora_results <- enrichGO(gene = cr_em_lost_gene_list,
                           universe = background_genes,
                           keyType = "ENSEMBL",
                           OrgDb = org.Hs.eg.db,
                           ont = "BP",
...
Now if we look at the types of genes that are identified uniquely by using the `cr-like-em` resolution in Alevin-fry we see detection of RNA processing genes and metabolism related genes. 
These are probably ribosomal genes and other related genes based on looking at the few unique genes that are higly expressed and noting that those are ribosomal genes. 
