...
```{r}
make_trace_df <- function(workdir, file_ext = ".trace.txt"){
  setwd(workdir)
  workdir_files <- list.files(workdir)
  workdir_list <- lapply(workdir_files, function(x) readr::read_tsv(x))
  # remove extension from trace file names
  names(workdir_list) <- gsub(file_ext, "", workdir_files)
  # create list of data frames for all trace files present in directory 
