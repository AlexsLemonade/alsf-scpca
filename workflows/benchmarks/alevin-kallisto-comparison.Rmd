...
rm(alevin_cell_qc, kallisto_cell_qc)
```
...
```{r}
cell_counts <- cell_qc %>%  
  dplyr::count(index_content, cell_id, sample)
...
```{r}
# note common cells are from either sample
common_cells <- common_cells_cdna[common_cells_cdna %in% common_cells_txome]
...
feature_qc <- dplyr::bind_rows(alevin_feature_qc, kallisto_feature_qc)
rm(alevin_feature_qc, kallisto_feature_qc)
```
...
feature_compare <- feature_qc %>%
  dplyr::filter(gene_id %in% detected_features,
                decoy == "no") %>%
  tidyr::pivot_wider(id_cols = c(gene_id, sample, index_content),
                     names_from = tool,
                     values_from = mean)
```
...
```{r}
feature_compare %>% 
  dplyr::group_by(sample, index_content) %>%
  dplyr::summarize(ak_cor = cor(alevin, kallisto, method = "spearman"))
```
```{r}
ggplot(feature_compare, aes(x = alevin, y = kallisto)) +
  geom_point(alpha = 0.1) + 
  facet_grid(sample ~ index_content) + 
  scale_x_log10() + 
  scale_y_log10() + 
  labs(x = "Alevin mean expression", y = "Kallisto mean expression")
```
I would say these correlations are perhaps surprisingly weak?
Quite honestly, I'm not sure what the exact conclusions should be here...
