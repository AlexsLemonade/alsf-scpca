...
With that file in the directory, you can test any changes with `terraform plan` and then implement them on AWS with `terraform apply`. This action will always involve infrastructure change for the compute environment (timestamp based name). This means you will always see `terraform plan` output similar to this `Plan: 3 to add, 3 to change, 3 to destroy.` even in case you haven't made any changes.
